<?php
/**
 * Content template for single property pages
 * Used via the_content filter (Estatik's approach)
 * This file contains only the content, no header/footer
 * 
 * NOTE: When called via the_content filter, we're already in the loop
 * so we use get_the_ID() directly instead of have_posts()/the_post()
 */
if (!defined('ABSPATH')) {
    exit;
}

// Get the current post ID (we're already in the loop via the_content filter)
$post_id = get_the_ID();
if (!$post_id) {
...
if (!$post_id) {
    // No post found
    echo '<div class="single-property"><div class="container" style="padding: 20px;"><h1>' . esc_html__('Property Not Found', 'realestate-booking-suite') . '</h1><p>' . esc_html__('Sorry, the property you\'re looking for doesn\'t exist or has been removed.', 'realestate-booking-suite') . '</p></div></div>';
    return;
}
...
<div class="single-property">
        ...
        $price_note     = get_post_meta($post_id, '_property_price_note', true);
        $call_for_price = get_post_meta($post_id, '_property_call_for_price', true);

        $bedrooms       = get_post_meta($post_id, '_property_bedrooms', true);
        $bathrooms      = get_post_meta($post_id, '_property_bathrooms', true);
        $half_baths     = get_post_meta($post_id, '_property_half_baths', true);
        $total_rooms    = get_post_meta($post_id, '_property_total_rooms', true);
        $floors         = get_post_meta($post_id, '_property_floors', true);
        $floor_level    = get_post_meta($post_id, '_property_floor_level', true);
        $area_sqft      = get_post_meta($post_id, '_property_area_sqft', true);
        $lot_size_sqft  = get_post_meta($post_id, '_property_lot_size_sqft', true);
        $year_built     = get_post_meta($post_id, '_property_year_built', true);
        $year_remodeled = get_post_meta($post_id, '_property_year_remodeled', true);

        $property_type      = get_post_meta($post_id, '_property_type', true);
        $property_status    = get_post_meta($post_id, '_property_status', true);
        $property_condition = get_post_meta($post_id, '_property_condition', true);
        ...
        $zip          = get_post_meta($post_id, '_property_zip', true);
        $country      = get_post_meta($post_id, '_property_country', true);
        ...
        $map_iframe   = get_post_meta($post_id, '_property_map_iframe', true);
        // Try alternative field names
        if (!$map_iframe) {
            $map_iframe = get_post_meta($post_id, '_property_custom_map_iframe', true);
        }
        if (!$map_iframe) {
            $map_iframe = get_post_meta($post_id, '_property_map_embed', true);
        }
        if (!$map_iframe) {
            $map_iframe = get_post_meta($post_id, '_property_google_map_iframe', true);
        }
        $hide_address = get_post_meta($post_id, '_property_hide_address', true);

        // Get features and amenities - COMPLETE FIX: Always return strings, never arrays
        $features_raw  = get_post_meta($post_id, '_property_features', true);
        ...
        $heating           = get_post_meta($post_id, '_property_heating', true);
        $cooling           = get_post_meta($post_id, '_property_cooling', true);
        $basement          = get_post_meta($post_id, '_property_basement', true);
        $roof              = get_post_meta($post_id, '_property_roof', true);
        $exterior_material = get_post_meta($post_id, '_property_exterior_material', true);
        $floor_covering    = get_post_meta($post_id, '_property_floor_covering', true);
        ...
        // Nearby features
        $nearby_schools     = get_post_meta($post_id, '_property_nearby_schools', true);
        $nearby_shopping    = get_post_meta($post_id, '_property_nearby_shopping', true);
        $nearby_restaurants = get_post_meta($post_id, '_property_nearby_restaurants', true);
        ...
        $get_field_with_default = function($meta_key, $default) use ($post, $post_id) {
        ...
        $video_embed              = get_post_meta($post_id, '_property_video_embed', true);

        // Agent data
        $agent_name              = get_post_meta($post_id, '_property_agent_name', true);
        $agent_phone             = get_post_meta($post_id, '_property_agent_phone', true);
        $agent_email             = get_post_meta($post_id, '_property_agent_email', true);
        $agent_photo             = get_post_meta($post_id, '_property_agent_photo', true);
        ...
        // Get property badges
        $property_badges = get_post_meta($post_id, '_property_badges', true);
        ...
        $property_statuses  = get_the_terms($post_id, 'property_status');
                ...
                <div class="tabs no-print">
                    ...
                    <div class="tabs-content">
                            ...
                            <div class="details-grid">
                                <div class="space-y-3">
                                    <div class="detail-item">
                                        ...
                                        <span class="detail-value"><?php echo esc_html($post_id); ?></span>
                                    </div>
                                    <?php if ($property_type): ?>
                                    <div class="detail-item">
                        ...
                        <div id="location-tab" class="tab-content">
                            ...
                            $map_id = 'resbs-property-map-' . $post_id;
